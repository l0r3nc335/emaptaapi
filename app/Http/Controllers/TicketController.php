...
class TicketController extends Controller
{
    ...
    public function projectsReport(Request $request){
        try {
            $year = $request->searchYear == "" ?  date("Y") : $request->searchYear;
            $month = $request->searchMonth == "" ?  date("m", strtotime(date("M")) ) : $request->searchMonth;

            $ticketReport = Ticket::ticketReports($year,$month);
            ...
            $ticketReport[0]->ReportMonth =  date("F", strtotime($year."-" . $ticketReport[0]->ReportMonth . "-01") );
            $response->data = $ticketReport;

            $response->pagination = new StdClass();
            $response->pagination->total = 1;
            $response->pagination->first_item = 1;
            $response->pagination->last_item = 1;
    ...
    public function projectsReportYearMonth(Request $request){
        try {
            ...
            $ticketYearMonth[0]->ticketYears = $years;
    ...
    {
        $ticket = Ticket::ticketReports('2021','4');
